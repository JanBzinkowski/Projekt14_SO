...
void zamowienie(SharedMem *shared_mem_flags) {
	...
	auto *table_array = static_cast<Table *>(mmap(nullptr, shared_mem_flags->tables_array_size, PROT_READ, MAP_SHARED, shared, sizeof(SharedMem)));
	...
	if (index == array_size) {
		//zwrot brak wolnych miejsc.
	}
	//wysłanie wiadomości o nr stolika i realizacja zamowienia
}
